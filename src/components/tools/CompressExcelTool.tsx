...
export default function CompressExcelTool() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (doc.name.toLowerCase().endsWith(".csv")) {
          ...
          resultBlob = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 9 } });
      } else {
          // For XLSX
          ...
          for (const relativePath of files) {
            ...
            if (relativePath.startsWith("xl/media/") && (relativePath.endsWith(".jpeg") || relativePath.endsWith(".jpg") || relativePath.endsWith(".png"))) {
          ...
          resultBlob = await newZip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 9 } });
      }

      const url = URL.createObjectURL(resultBlob);
      const a = document.createElement("a");
      a.href = url;
      a.download = doc.name.toLowerCase().endsWith(".csv") ? `compressed_${doc.name}.zip` : `compressed_${doc.name}`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e: unknown) {
      setError((e instanceof Error ? e.message : "Failed to compress Excel/CSV document."));
    } finally {
      setProcessing(false);
    }
  };
  ...
  if (!doc) {
    return (
      <FileDrop
        ...
        multiple={false}
        icon="compress"
        title={<>Drop an Excel/CSV file or <span className="em">browse</span></>}
        sub="Best for shrinking images inside Excel files."
      />
    );
  }
  ...
  return (
    <div className="stack" style={{ gap: "var(--s-5)" }}>
      ...
      <Banner kind="info" title="How this works">
        Compression extracts images from the spreadsheet file and re-compresses them. It will shrink image-heavy Excel files but won&apos;t help pure text files. CSV files are simply ZIP-compressed.
      </Banner>
